...
Read the material and then ask me what I'd like to know about {topic}."""})
        ...
        prompt = None
        if stop_generation:
            # Force a cache reset after interruption to guarantee state consistency
            # for the next turn, as incremental trimming can be unreliable
            # with complex model states (e.g., MTP, thinking layers).
            prompt_cache = make_prompt_cache(
                model,
                args.max_kv_size,
                turbo_kv_bits=args.turbo_kv_bits,
            ...
            _cache_stale = True
            rprint("[INFO] Cache reset after interruption for stability.")
